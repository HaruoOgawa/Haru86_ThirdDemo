...
else
{
    ...
    vec3 col = vec3(0.0),ro=vec3(0.0,0.0,1.5),ta=vec3(0.0),cdir=normalize(ta-ro),
    cside=normalize(cross(vec3(0.0,1.0,0.0),cdir)),cup=normalize(cross(cdir,cside)),
    ...
    //if(mr.d>dmin)
    {
        vec3 p;
        bool IsSea=false;
        CreateSeaHightMap(ro,rd,p,IsSea);
        vec3 dist = p-ro;
