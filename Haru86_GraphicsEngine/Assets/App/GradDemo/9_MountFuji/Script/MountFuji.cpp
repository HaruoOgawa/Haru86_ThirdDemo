...
namespace app {
	...
	void MountFuji::UpdateTimeLine(float time)
	{
		...
		if (SceneIndex == 9)
		{
			...
			else if (time >= 218.0f && time < 222.0f) { CameraPos.z -= (time - 218.0f); m_UseTextIndex = 2; m_Alpha = sin(glm::clamp((time >= 221.0f) ? 222.0f - time : time - 218.0f, 0.0f, 1.0f) * pi * 0.5f); }
			else if (time >= 222.0f && time < 226.0f) { CameraPos.z -= (time - 218.0f); m_UseTextIndex = 3; m_Alpha = sin(glm::clamp((time >= 225.0f) ? 226.0f - time : time - 222.0f, 0.0f, 1.0f) * pi * 0.5f); }
