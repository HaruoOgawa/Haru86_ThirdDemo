/* File generated with Shader Minifier 1.3.1
 * http://www.ctrl-alt-test.fr
 */

// E:\CppDev\GradDemo\Haru86_GraphicsEngine\GraphicsEngine\Graphics\Shader\Standard_Frag.h
"#version 430\n"
 "#extension GL_ARB_separate_shader_objects:enable\n"
 "layout(location=0) in vec2 in_uv;"
 "layout(location=1) in vec4 in_WorldVertexPos;"
 "layout(location=2) in vec4 in_WorldNormal;"
 "layout(location=3) flat in int in_gl_InstanceID;"
 "out vec4 out_Color;"
 "uniform int _UseColor;"
 "uniform vec4 _Color;"
 "uniform int _UseLighting;"
 "uniform vec3 _LightDir,_LightPos,_WorldCameraPos;"
 "uniform int _UseEnvColor;"
 "uniform vec4 _EnvColor;"
 "uniform int _UseMainTex,_UseMainEditorTex;"
 "uniform sampler2D _MainTex;"
 "uniform int _UseMainCube;"
 "uniform samplerCube _MainCube;"
 "uniform int _UseRim,_IsRimMul;"
 "uniform vec4 _RimColor;"
 "uniform float _RimPower,_RimMulVal,_EditorOffset;"
 "void main()"
 "{"
   "vec4 i=vec4(vec3(0.),1.);"
   "if(_UseColor==1)"
     "i=_Color;"
   "else"
     " if(_UseMainTex==1)"
       "{"
         "vec4 v=texture(_MainTex,vec2(in_uv.x,1.-in_uv.y));"
         "i=_Color*v.x;"
       "}"
     "else"
       " if(_UseMainEditorTex==1)"
         "{"
           "vec4 v=texture(_MainTex,vec2(in_uv.x,in_uv.y+_EditorOffset));"
           "i=v;"
         "}"
       "else"
         " if(_UseMainCube==1)"
           "{"
             "vec3 v=-normalize(_WorldCameraPos-in_WorldVertexPos.xyz),r=normalize(reflect(v,in_WorldNormal.xyz));"
             "i.xyz=texture(_MainCube,r).xyz;"
           "}"
         "else"
           " i=vec4(1.);"
   "vec4 v=vec4(0.,0.,0.,1.);"
   "if(_UseEnvColor==1)"
     "v=_EnvColor;"
   "if(_UseLighting==1)"
     "{"
       "vec3 r=normalize(_LightPos-in_WorldVertexPos.xyz);"
       "float n=max(0.,dot(in_WorldNormal.xyz,r));"
       "i.xyz*=n;"
       "i.xyz+=v.xyz;"
       "vec3 l=-1.*normalize(in_WorldVertexPos.xyz-_WorldCameraPos),z=normalize(l+r);"
       "float u=pow(max(0.,dot(in_WorldNormal.xyz,z)),60.);"
       "u=min(1.,u);"
       "i.xyz+=vec3(1.)*u;"
     "}"
   "if(_UseRim==1)"
     "{"
       "vec3 r=-normalize(in_WorldVertexPos.xyz-_WorldCameraPos);"
       "float u=1.-dot(in_WorldNormal.xyz,r);"
       "u=pow(u,_RimPower);"
       "u=max(0.,u);"
       "u*=_RimMulVal;"
       "if(_IsRimMul==1)"
         "i*=_RimColor*u;"
       "else"
         " i=_RimColor*u;"
     "}"
   "out_Color=i;"
 "}",

